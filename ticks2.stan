...
parameters {
  ...
  real<lower=0> sig_tot;        // Total variance
  real<lower=0,upper=1> p_gamma; // Proportion of variance to location effect
}
transformed parameters {
  ...
  sig_gamma = sqrt(p_gamma * sig_tot);
  sig_beta = sqrt((1 - p_gamma) * sig_tot);
}
model {
  // Hyperpriors
  sig_tot ~ student_t(3, 0, 1);
  p_gamma ~ beta(0.1, 0.1);
